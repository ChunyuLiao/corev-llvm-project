...
DecodeStatus RISCVDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
  ...
  if ((Bytes[0] & 0x3) == 0x3) {
    ...
    if (STI.getFeatureBits()[RISCV::FeatureExtXCoreVAluV2]) {
      LLVM_DEBUG(dbgs() << "Trying XCoreVAluV2 custom opcode table:\n");
      Result = decodeInstruction(DecoderTableXCoreVAluV232, MI, Insn, Address, this,
                                 STI);
      if (Result != MCDisassembler::Fail) {
        Size = 4;
        return Result;
